...
import tripleo.elijah_fluffy_congenial.diagnostic.ExceptionDiagnostic;
...
public class Operation2<T> {
	...
	@Contract(pure = true)
	public static <T> @NotNull Operation2<T> failure(final Exception aException) {
		final Operation2<T> op = new Operation2<>(null, new ExceptionDiagnostic(aException), Mode.FAILURE);
		return op;
	}
