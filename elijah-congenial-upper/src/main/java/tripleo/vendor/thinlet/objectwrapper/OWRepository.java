 ...
 * counterpart OWObject released by the {@link OWObject#release()} call, to avoid leak memory.
...
public final class OWRepository {
	...
	private static final Object[] @NotNull []     fcomponents = {
	...
	private Object                   fobjectWrappers;
	private java.lang.reflect.Method fobjectWrappersGet;
	private java.lang.reflect.Method fobjectWrappersPut;
	private java.lang.reflect.Method fobjectWrappersRemove;


	/**
	 * build a new OWThinlet, for managing Object Wrapper thinlet widget
	 */
	public OWRepository() {
		super();
		try {
			Class<?> clazz = Class.forName("java.util.WeakHashMap");
			fobjectWrappers       = clazz.getConstructor().newInstance();
	 ...
	 */
	private static Class<?> findWrapperClassForComponent(String componentName) {
        for (Object[] fcomponent : fcomponents)
            if (fcomponent[0].equals(componentName))
                return (Class<?>) fcomponent[1];
		throw new IllegalArgumentException(componentName + " unknown");
	}
