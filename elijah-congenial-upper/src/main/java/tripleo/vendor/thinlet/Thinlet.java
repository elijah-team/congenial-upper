...
public class Thinlet extends Container implements Runnable, Serializable {
		...
		if ("combobox" == classname) {
			...
			if (scrollable) { // set scrollpane areas
				if ("panel" == classname) {
					...
					scrollable = layoutScroll(component, contentwidth, contentheight, 3 + titleheight, 3, 3, 3, true,
					                          0);
				}
			}
				...
				if (iclass == "dialog") {
					...
					if (get(comp, "bounds") == null)
						setRectangle(comp, "bounds", Math.max(0, (bounds.width - d.width) / 2), Math.max(0,
						                                                                                 (bounds.height - d.height) / 2),
						             Math.min(d.width, bounds.width), Math.min(
	...
	private void paint(@NotNull Graphics g, int clipx, int clipy, int clipwidth, int clipheight, Object component,
		...
		if ("label" == classname) {
			...
			paintScroll(component, ":combolist", pressed, inside, focus, enabled, g, clipx, clipy, clipwidth,
			            clipheight);
		} else if (("textfield" == classname) || ("passwordfield" == classname)) {
			...
			for (Object tab = get(component, ":comp"); tab != null; tab = get(tab, ":next")) {
				...
				if (selected != i) {
					...
					paint(tab, (placement == "left") ? r.width - 1 : 0, stacked ? (r.y + r.height - 1)
							      ...
							      : horizontal ? (bounds.height - r.height + 1) : bounds.height, g, true, true, true,
					      true,
					      enabled ? 'e' : 'd');
		...
		if ("textarea" == classname) {
			...
			for (Object item = get(component, ":comp"), next = null; item != null; item = next) {
				...
				if ("tree" != classname) {
					...
					if (!expanded) {
						for (Object node = item; (node != component) && ((next = get(node,
						                                                             ":next")) == null); node = getParent(
								node))
							;
					}
				}
	...
	private @Nullable Object getMenu(Object component, Object part, boolean forward, boolean popup) {
		...
		for (int i = 0; i < 2; i++) { // 0: next to last, 1: first to previous
			for (Object item = (i == 0) ? get(part, ":next") : get(popup ? get(component, "menu") : component,
			                                                       ":comp"); (i == 0) ? (item != null)
	...
	private int getCaretLocation(Object component, int x, int y, boolean multiline, boolean hidden) {
		Font        currentfont = (Font) get(component, "font");
		FontMetrics fm          = getFontMetrics((currentfont != null) ? currentfont : font);
		char[]      chars       = multiline ? ((char[]) get(component, ":text")) : getString(component, "text",
		                                                                                     "").toCharArray(); // update
		// it
		int linestart = 0;
		if (multiline) {
	...
	private void handleMouseEvent(int x, int y, int clickcount, boolean shiftdown, boolean controldown,
		...
		if (("list" == classname) || ("table" == classname) || ("tree" == classname)) {
			...
			if (editable && (part == null)) { // textfield area
				Image icon = null;
				int left = ((id == MouseEvent.MOUSE_PRESSED) && ((icon = getIcon(component, "icon",
				                                                                 null)) != null)) ? icon
						.getWidth(this)
						: 0;
			...
			if (((id == MouseEvent.MOUSE_ENTERED) || (id == MouseEvent.MOUSE_EXITED)) && (part != null)
				...
				if ((part != insidepart)
						&& ((insidepart == null) || ((insidepart instanceof Object[]) && (getClass(
						insidepart) != "menu")))) {
	...
	private boolean checkMnemonic(@Nullable Object component, boolean parent, Object checked, int keycode, int modifiers) {
		if ((component == null) || !getBoolean(component, "visible", true) || !getBoolean(component, "enabled",
		                                                                                  true)) { //+
			// enabled
			// comp
			// in
		...
		if ("label" == classname) {
			...
			if (((modifiers == 0) && (((keycode == KeyEvent.VK_ENTER) && (get(component,
			                                                                  "type") == "default")) || ((keycode == KeyEvent.VK_ESCAPE) && (get(
