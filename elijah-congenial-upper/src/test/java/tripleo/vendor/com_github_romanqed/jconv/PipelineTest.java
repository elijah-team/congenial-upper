package tripleo.vendor.com_github_romanqed.jconv;

import org.jetbrains.annotations.NotNull;
import org.junit.jupiter.api.Assertions;
import org.junit.jupiter.api.Test;
import tripleo.vendor.com_github_romanqed.jfunc.Runnable2;


public final class PipelineTest {
	public void testBuilder(@NotNull PipelineBuilder<Context> builder) throws Throwable {
		var task = builder
				.add((context, next) -> {
					context.value += 1;
					next.run(context);
		...
		task.run(context);
		// assertThat(context.value.intValue()).is(equalTo(10));
		Assertions.assertEquals(10, context.value);
	}
	...
	public void testExceptionHandling(@NotNull PipelineBuilder<Context> builder) {
		...
		Assertions.assertAll(
				...
				() -> {
					...
					Assertions.assertEquals(exception, e.getCause());
					Assertions.assertEquals(exception, context.exception);
				}
		);
	}
	...
	public void testShortCircuiting(PipelineBuilder<Context> builder) {
		...
		Assertions.assertAll(
				() -> {
					context.value = 0;
					task.run(context);
					Assertions.assertEquals(0, context.value);
				},
				() -> {
					context.value = -10;
					task.run(context);
				...
				() -> {
					context.value = 15;
					task.run(context);
					Assertions.assertEquals(5, context.value);
				}
		);
	}
